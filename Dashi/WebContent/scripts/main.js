(function() {
	 ...
	 */
	function init() {
		// Register event listeners ,  Capture and Bubble, default is bubble false
		...
		$('change-zipcode-btn').addEventListener('click', changeZipcode); 
...
//		});
	...
	function initGeoLocation() {
		if (navigator.geolocation) { // window.navigator.userAgent
	...
	function register() {
		...
		password = md5(username + md5(password));
		var firstname = $('register-first-name').value;
		var lastname = $('register-last-name').value;
	...
	function showRegisterError() {
		$('register-error').innerHTML = 'Username has been taken';
	}
	...
	function ajax(method, url, data, callback, errorHandler) {
		...
		xhr.open(method, url, true); //true 异步操作

		xhr.onload = function() {
			switch (xhr.status) {
			case 200: // 203是已经缓存在服务器了从缓存来的
	...
	function loadNearbyRestaurants() {
		//var zip_code = prompt("Please Enter Your Zipcode", "92617");
		console.log('loadNearbyRestaurants');
		activeBtn('nearby-btn');
		
		var zc = '92617'; 
		
			if ($('zip-code-number').value < 0 || $('zip-code-number').value > 99999) {
				alert("invalid zipcode");
			} else {
				zc = $('zip-code-number').value.toString();
			}
		...
		// The request parameters
		var url = './restaurants';
		var params = 'user_id=' + user_id + '&zip_code=' + zc;
